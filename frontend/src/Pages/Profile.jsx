...
export default function Profile() {
    ...
    const [docId, setDocId] = useState(null); // Store the actual Firestore Document ID
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            if (admin?.uid) {
                try {
                    // 1. Try fetching by UID first (Best practice)
                    ...
                    if (docSnap.exists()) {
                        ...
                        // 2. Fallback: Query by email if UID doesn't match Doc ID (handling random IDs from addDoc)
                    ...
                    if (userData) {
                        setDocId(foundDocId); // Store the correct Doc ID
                        setFormData(prev => ({
                            ...prev,
                        ...
                        // 3. Last Resort: Use Auth data only
                        setFormData(prev => ({
                            ...prev,
                            email: admin.email,
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            // Ensure we have a valid Doc ID to update
            let targetDocId = docId;
            if (!targetDocId) {
                // Last ditch effort: Query by email again if state was lost or not found initially
                const q = query(collection(db, "admins"), where("email", "==", admin.email));
                const querySnapshot = await getDocs(q);
                if (!querySnapshot.empty) {
                    targetDocId = querySnapshot.docs[0].id;
                } else {
                    // Critical Error: Admin document truly doesn't exist in Firestore
                    // Optionally create one here? For now, throw error.
    ...
    return (
        <div className="font-serif min-h-full w-full flex flex-col items-center bg-[#1c1917] text-[#e7e5e4]">
            {/* Header */}
            <div className="w-full bg-[#0c0a09] border-b-4 border-[#292524] py-10 px-6 shadow-2xl">
                <div className="max-w-6xl mx-auto">
                    ...
                    <p className="text-[#a8a29e] mt-2 tracking-wide uppercase text-xs">Manage Profile</p>
                </div>
            </div>

            <div className="max-w-6xl w-full px-6 py-12">
                <form onSubmit={handleSave} className="flex flex-col lg:flex-row gap-12 items-start">
                    
                    {/* Left Column: Avatar Upload */}
                    <div className="w-full lg:w-1/3 flex flex-col items-center sticky top-24">
                        <div className="relative group p-1 bg-[#0c0a09] rounded-full border-2 border-[#44403c] shadow-2xl overflow-hidden mb-6">
                            <div className="w-64 h-64 rounded-full overflow-hidden relative">
                                ...
                                <label className="absolute inset-0 bg-[#0c0a09]/80 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all cursor-pointer border-4 border-dashed border-[#d4af37]/30 rounded-full m-2">
                                    ...
                                    <span className="text-[#e7e5e4] text-[10px] uppercase font-bold tracking-[0.2em]">Change Avatar</span>
                                    <input type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
                                </label>
                            </div>
                        </div>
                        <div className="text-center">
                            <h2 className="text-2xl font-bold text-[#d4af37] tracking-widest uppercase">{formData.firstName} {formData.lastName}</h2>
                            <p className="text-[#57534e] text-xs uppercase tracking-[0.2em] mt-1">Change your Avatar here</p>
                        </div>
                    </div>

                    {/* Right Column: Form Fields */}
                    <div className="flex-1 w-full space-y-8">
                        <div className="bg-[#292524] p-8 rounded-sm border border-[#44403c] shadow-2xl relative overflow-hidden">
                            ...
                            <h3 className="text-[#d4af37] font-bold uppercase tracking-widest text-lg mb-8 border-b border-[#44403c] pb-4 flex items-center gap-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        className="w-full bg-[#0c0a09] border border-[#44403c] p-4 rounded text-[#e7e5e4] focus:border-[#d4af37] outline-none transition-all shadow-inner"
                                ...
                                <div className="space-y-2 md:col-span-2">
                                    ...
                                    <div className="relative">
                                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-[#44403c]" size={18} />
                                        <input 
                                            type="email"
                                            className="w-full bg-[#0c0a09] border border-[#44403c] pl-12 pr-4 py-4 rounded text-[#e7e5e4] focus:border-[#d4af37] outline-none transition-all shadow-inner"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input 
                                            ...
                                            className="w-full bg-[#0c0a09] border border-[#44403c] p-4 pr-12 rounded text-[#e7e5e4] focus:border-[#d4af37] outline-none transition-all shadow-inner"
                                            placeholder="••••••••"
                        ...
                        <div className="flex justify-end pt-4">
                            <button 
                                type="submit"
                                disabled={saving}
                                className="bg-[#2c241b] text-[#d4af37] px-12 py-4 rounded font-bold uppercase tracking-[0.2em] border border-[#d4af37]/50 hover:bg-[#d4af37] hover:text-[#1c1917] transition-all flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed shadow-xl hover:shadow-[0_0_20px_rgba(212,175,55,0.3)]"
                            >
